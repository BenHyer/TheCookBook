...
#pragma warning disable EF1002 // Schema identifiers cannot be parameterized
            await auditDb.Database.ExecuteSqlRawAsync($"CREATE SCHEMA IF NOT EXISTS \"{escapedSchema}\"");

            await auditDb.Database.ExecuteSqlRawAsync($"""
                CREATE TABLE IF NOT EXISTS "{escapedSchema}"."AuditLogs" (
                ...
                CREATE INDEX IF NOT EXISTS "IX_AuditLogs_TimestampUtc"
                    ON "{escapedSchema}"."AuditLogs" ("TimestampUtc");

                CREATE INDEX IF NOT EXISTS "IX_AuditLogs_UserId"
                    ON "{escapedSchema}"."AuditLogs" ("UserId");
                """);
